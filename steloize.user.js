...
(function() {
  ...
  class FontApplier {
    ...
    loadFont(fontFamily) {
      const fontFace = new FontFace(fontFamily, `url(https://steloize-fontfaces.000webhostapp.com/${fontFamily}.woff)`)
  ...
  cafeMain.addEventListener('load', () => {
    ...
    let subtitle;
    let subtitleElement;
    if ((subtitleElement = cafeMain.contentDocument.body.querySelector('#sub-tit .info_tit h3'))) {
    ...
    if (subtitle === undefined) {
      ...
      const sortForms = Array.from(cafeMain.contentDocument.querySelectorAll('.sort_form a'))
      let sortMethod = 'sort_list';

      mainFontApplier.applyFontsBySelector('#upperArticleList .inner_list .article')
      ...
      switch (sortMethod) {
        ...
        case 'sort_video':
          mainFontApplier.applyFontsBySelector('.article-album-movie-sub .tit_txt')
          mainFontApplier.applyFontsBySelector('.article-album-movie-sub td.p-nick a')
          break

        case 'sort_album':
          mainFontApplier.applyFontsBySelector('.article-album-sub .inner span')
          mainFontApplier.applyFontsBySelector('.article-album-sub td.p-nick a')
          break

        case 'sort_list':
          mainFontApplier.applyFontsBySelector('.article-board:nth-child(2) .inner_list .article')
          mainFontApplier.applyFontsBySelector('.article-board:nth-child(2) .p-nick a')
          break
      }
    }
  })
